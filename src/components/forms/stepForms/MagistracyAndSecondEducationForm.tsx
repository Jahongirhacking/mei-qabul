...
import {
  MasterAndSecondEducationAdmissionDto,
  useCreateMasterAndSecondEduAdmission,
  useGetOldEdu,
  useUpdateMasterAndSecondEduAdmission
...
interface FormValues
  extends Omit<MasterAndSecondEducationAdmissionDto, 'diploma' | 'admissionTypeId'> {
  diploma?: GeneratedFileType[] | string
}

export function MagistracyAndSecondEducationForm() {
  ...
  const { oldEdu, isOldEduLoading } = useGetOldEdu<MasterAndSecondEducationAdmissionDto>()
  ...
  const { create, isCreating } = useCreateMasterAndSecondEduAdmission({
    onSuccess() {
      nextStep()
    }
  })

  const { update, isUpdating } = useUpdateMasterAndSecondEduAdmission({
    onSuccess() {
      nextStep()
    }
  })
  ...
  const submit = (values: FormValues) => {
    const dto: MasterAndSecondEducationAdmissionDto = {
      ...values,
    ...
    <div>
      <Form initialValues={oldEdu} autoComplete="off" layout="vertical" onFinish={submit}>
        <div className="mt-6">
          <div>
            <Form.Item
              label="Oliy ta'lim muassasangiz nomi"
              name="university"
              rules={[{ required: true }]}
            >
            ...
            <Form.Item
              label="Bakalavriat ta'lim yo'nalishi"
              name="speciality"
              rules={[{ required: true }]}
            >
              <TextInput placeholder="Bakalavriat ta'lim yo'nalishini kiriting" />
            </Form.Item>
            ...
            <FileFormItem label="Bakalavriat diplomi" name="diploma" rules={[{ required: true }]} />
          </div>
        </div>
