...
export default function TechnicalContractsPage() {
  ...
  const { download, isDownload } = useDownloadExcelContracts({
    onSuccess: (data) => {
      openLink(URL.createObjectURL(data), 'Texnikum bitiruvchilari shartnomalari excel')
    }
  })

  return (
    <Container
      title="Texnikum bitiruvchilari shartnomalari"
      extra={
        <Space>
