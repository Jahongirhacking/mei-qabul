...
export default function FailedExams({ examStatus }: IExamsParams) {
  ...
  const { download, isDownload } = useDownloadExcelExamsApplications({
    onSuccess: (data) => {
      openLink(URL.createObjectURL(data), "Imtihondan o'tmaganlar excel")
    }
  })
  ...
  return (
    <Container
      padding="p-0"
      title="Imtihondan o'tolmaganlar"
      extra={
        <Space>
          <SelectInput
            style={{ width: '250px' }}
